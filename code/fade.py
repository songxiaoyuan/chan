...
class kmesg_class(object):
	"""docstring for kmesg"""
	def __init__(self,open_price,close_price,high_price,low_price,time):
		super(kmesg_class, self).__init__()
...
class getline(object):
	...
	def __init__(self,param_dict):
		...
		self._listkmsg_array = [] #保存所有的生成的k线的信息。
		...
		self.zs_now_zoushilist=[]#进入中枢的走势【begin_val,over_val,timebegin,macd红，macd绿，timeend】
		self.zs_now_penlist=[]
		...
		if self._now_bar_num ==0:
			...
			bf.get_config_info(zs_now_dir_array,zs_index_array,quick_ema_array,slow_ema_array,self._diff_array,
		...
		if self._now_bar_num_short ==0:
			...
			bf.get_config_info(zs_now_dir_short_array,zs_index_short_array,quick_ema_array,slow_ema_array,self._diff_array_short,
	...
	def savemsg(self):
		...
		#self.over_fun()
		print 'save self.zs_index is   ',self.zs_index
		path = "../config/" + str(self._k_period)+"/"+str(self._config_file)
		bf.write_config_info(self.zs_now_dir,self.zs_index,self._quick_ema,self._slow_ema,
		...
		bf.write_config_info(self.zs_now_dir_short,self.zs_index_short,self._quick_ema_short,self._slow_ema_short,
		...
		if longorshort==1:
			...
			if (len(self._penmsg_list)==0) or (len(self._penmsg_list)>0 and len(self._parting_array)>=2 and float((self._parting_array[-2])[1])!=float((self._penmsg_list[-1])[0])):#有一对顶底 or 新的一笔
				...
				if ((int((self._penmsg_list[-1])[2])==1 and float((self._parting_array[-1])[1])>float((self._penmsg_list[-1])[1]))  or (int((self._penmsg_list[-1])[2])==0 and float((self._parting_array[-1])[1])<float((self._penmsg_list[-1])[1]))):
					...
					(self._penmsg_list[-1])[4]=float((self._penmsg_list[-1])[4])+(self._parting_array[-1])[3]#macd 红
					(self._penmsg_list[-1])[5]=float((self._penmsg_list[-1])[5])+(self._parting_array[-1])[4]#macd 绿
					(self._penmsg_list[-1])[6]=(self._parting_array[-1])[0]
					self._penmsg_listchangeflag=1
					#print 'pen change  hahah2 !!!!!    ',self._penmsg_list
	...
	def findzhongshu(self,longorshort):
		if longorshort==1:
			...
			#print 'have zhongshu  haha  ',self.zs_now_upvalue,'   ',self.zs_now_uptime,'   ',self.zs_now_downvalue,'   ',self.zs_now_downtime,'   ',self.zs_now_dir,'   ',self.zs_now_zoushilist
			if self.zs_index==0 and self._penmsg_listchangeflag==1:#如果现在没有中枢，生成新中枢
				...
				#self._penmsg_list[startvalue,overvalue,dddir,starttime,macd红，macd绿,overtime]  dddirs=1:笔方向向上   dddir=0:比方向向下
				#【begin,over,timebegin,macd红，macd绿，timeend】
				if self.zs_now_dir==1:#中枢方向向上
					...
					if float((self._penmsg_list[-3])[0])>float((self._penmsg_list[-1])[1]) and int((self._penmsg_list[-3])[2])==self.zs_now_dir and int((self._penmsg_list[-1])[2])==self.zs_now_dir :#判断是否有交集,有交集
						...
						# 	self.zs_now_zoushilist.append((self._penmsg_list[-4])[0])
						# 	self.zs_now_zoushilist.append((self._penmsg_list[-2])[1])
						# 	self.zs_now_zoushilist.append((self._penmsg_list[-4])[2])
						# 	self.zs_now_zoushilist.append((self._penmsg_list[-4])[3])
						# 	tempbarred=(self._penmsg_list[-4])[4]+(self._penmsg_list[-3])[4]+(self._penmsg_list[-2])[4]
						# 	tempbargreen=(self._penmsg_list[-4])[5]+(self._penmsg_list[-3])[5]+(self._penmsg_list[-2])[5]
						# 	self.zs_now_zoushilist.append(tempbarred)
						# 	self.zs_now_zoushilist.append(tempbargreen)
						# 	self.zs_now_zoushilist.append((self._penmsg_list[-2])[6])
						# else:#之前有进入中枢的笔，此处修改进入中枢结束，和macd值
						# 	self.zs_now_zoushilist[1]=(self._penmsg_list[-2])[1]
						# 	self.zs_now_zoushilist[4]=float(self.zs_now_zoushilist[4])+(self._penmsg_list[-2])[4]
						# 	self.zs_now_zoushilist[5]=float(self.zs_now_zoushilist[5])+(self._penmsg_list[-2])[5]
						# 	self.zs_now_zoushilist[6]=(self._penmsg_list[-2])[6]
						# 	
						if len(self.zs_now_zoushilist)==0:#之前没有进入中枢的笔，此处进入中枢的笔开始赋值
				...
				self._penmsg_listchangeflag=0
				if self.zs_now_dir==1:#中枢方向向上
					#print "leave zhongshu  up  ",self.zs_now_upvalue, '  ',self.zs_now_downvalue, '  ',self.zs_now_dir, '  ',self.zs_now_uptime, '  ',self.zs_now_downtime, '  ',self.zs_now_zoushilist
					#print 'leave zhongshu     ',self._penmsg_list
	...
	def over_fun(self):
		...
		self.mergek(tmp,1)
		#print "haha2"
		self.get_merge_line(self._listkmsg_array,self._updownlist,1)
		#print "haha3"
		...
		self.mergek(tmp,0)
		#print "haha6"
		self.get_merge_line(self._listkmsg_array_short,self._updownlist_short,0)
		#print "haha7"
	...
	def create_k(self,time,lastprice):
		...
		if self._k_bar_tick ==1:
			...
			tmp = kmesg(self._open_price,self._close_price,self._high_price,self._low_price,time)
			# tmp.append(self._open_price)
			# tmp.append(self._close_price)
			# tmp.append(self._high_price)
			# tmp.append(self._low_price)
	...
	def mergek(self,kmesg):
		...
		if not self._lastlastk_array:
			...
			self._updownlist.append(templist)
			return
		...
		if (float(kmesg[2])<=float(lastk[2]) and float(kmesg[3])>=float(lastk[3])) or (float(kmesg[2])>=float(lastk[2])and float(kmesg[3])<=float(lastk[3])):#当前k线需要和前一k线合并
			...
			if float(lastk[2])>float(kmesg[2]) and float(lastk[2])>float(lastlastk[2]) and float(lastk[3])>float(kmesg[3]) and float(lastk[3])>float(lastlastk[3]) and self._midkone>3:
				if self._dd_dir==0 or (self._dd_dir==1 and lastk[2]>self._dd_value) or self._dd_dir==-1:
					if len(self._updownlist)==0:#在前一个文件
						templist=[]
						templist.append(1)
					...
					((self._updownlist)[-1])[0]=1
					((self._updownlist)[-1])[1]=lastk[2]
					self._midkone=0
					self._dd_dir=1
					self._dd_value=lastk[2]
			...
			if float(lastk[2])<float(kmesg[2]) and float(lastk[2])<float(lastlastk[2]) and float(lastk[3])<float(kmesg[3]) and float(lastk[3])<float(lastlastk[3]) and self._midkone>3:
				if self._dd_dir==0 or (self._dd_dir==-1 and lastk[3]<self._dd_value) or self._dd_dir==1:
					...
					((self._updownlist)[-1])[0]=-1
					((self._updownlist)[-1])[1]=lastk[3]
				#self._updownlist[-1]=-1
					self._midkone=0
					self._dd_dir=-1
					self._dd_value=lastk[3]
			self._updownlist.append(templist)
		...
		if longorshort==1:
			...
			if parting == 0:
				...
				if len(self._parting_array) ==0:
					self._parting_array.append([time,lastprice,1,0,0])#最后一个数据是macd的值
					return 1
		...
		if longorshort==0:
			...
			if parting == 0:
				...
				if len(self._parting_array_short) ==0:
					self._parting_array_short.append([time,lastprice,1,0,0])
					return 1
	...
	def get_md_line(self,line):
		...
		if kmesg ==None:
			...
			if tmpbar>=0 and len(self._parting_array)!=0:
				...
				(self._parting_array[-1])[4]=(self._parting_array[-1])[4]+tmpbar
	...
	def write_data_to_file(self):
		...
		#######写长周期
		ret = []
		if len(self._listkmsg_array) !=  len(self._updownlist):
			print "len(self._listkmsg_array) !=  len(self._updownlist)"
			return
		...
		#######写短周期
		ret = []
		if len(self._listkmsg_array_short) !=  len(self._updownlist_short):
			print "len(self._listkmsg_array) !=  len(self._updownlist)"
			return
		...
		for x in xrange(0,len(self._listkmsg_array_short)):
			...
			tmp.append(self._updownlist_short[x])
...
if __name__=='__main__': 
	# main()
	k = kmesg_class(12,34,1,55,"12:01")
	print k._open_price
