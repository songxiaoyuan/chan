	...
	def findpen(self,longorshort):
		if longorshort==1:
			if len(self._parting_array)<2:
				return
			if (len(self._penmsg_list)==0) or (len(self._penmsg_list)>0 and float((self._parting_array[-2])[1])!=float((self._penmsg_list[-1])[0])):#有一对顶底 or 新的一笔
				templist=[]
	...
	def findzhongshu(self,longorshort):
		if longorshort==1:
			...
			if len(self._penmsg_list)==0:#没有笔
				return
			if len(self._penmsg_list)==1 and self.zs_now_upvalue==0:#只有一笔，判断中枢方向,这个方向需要提前给
				#print self._penmsg_list
			...
			if self.zs_now_upvalue==0:#如果现在没有中枢，生成新中枢
				...
				if self.zs_now_dir==1:#中枢方向向上
					if  (self._penmsg_list[-3])[0]<(self._penmsg_list[-1])[1] and (self._penmsg_list[-3])[2]==self.zs_now_dir and (self._penmsg_list[-1])[2]==self.zs_now_dir :#判断是否有交集
					...
					elif (self._penmsg_list[-3])[0]>=(self._penmsg_list[-1])[1] and (self._penmsg_list[-3])[2]==self.zs_now_dir and (self._penmsg_list[-1])[2]==self.zs_now_dir :
						#self._penmsg_list[startvalue,overvalue,dddir,starttime,macd红，macd绿,overtime]  dddirs=1:笔方向向上   dddir=0:比方向向下
						#print "zoushilist      ",self.zs_now_zoushilist
						#print "_penmsg_list      ",self._penmsg_list
						if len(self.zs_now_zoushilist)==0:#之前没有进入中枢的笔，此处进入中枢的笔开始赋值
					...
					if (self._penmsg_list[-3])[0]>(self._penmsg_list[-1])[1] and (self._penmsg_list[-3])[2]==self.zs_now_dir and (self._penmsg_list[-1])[2]==self.zs_now_dir :#判断是否有交集,有交集
						#self._penmsg_list[startvalue,overvalue,dddir,starttime,macd红，macd绿,overtime]  dddirs=1:笔方向向上   dddir=0:比方向向下
					...
					elif (self._penmsg_list[-3])[0]<=(self._penmsg_list[-1])[1] and (self._penmsg_list[-3])[2]==self.zs_now_dir and (self._penmsg_list[-1])[2]==self.zs_now_dir :#判断是否有交集，没有交集
						# if len(self.zs_now_zoushilist)==0:#之前没有进入中枢的笔，此处进入中枢的笔开始赋值
		...
		if longorshort==0:
			...
			if len(kmesg) ==0:
				...
				if tmpbar>=0 and len(self._parting_array_short)!=0:
					...
					(self._parting_array_short[-1])[4]=(self._parting_array_short[-1])[4]+tmpbar
...
def main():
	...
	#date3 = [20171016,20171017,20171018]
	date = date1+date2
